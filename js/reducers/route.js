...
import { globalHomeNav } from '../components/home';
import { PUSH_NEW_ROUTE, POP_ROUTE, POP_TO_ROUTE, REPLACE_ROUTE, REPLACE_OR_PUSH_ROUTE,PUSH_NEW_ROUTE_HOME,POP_ROUTE_HOME } from '../actions/route';
...
export default function (state:State = initialState, action:Action): State {
    ...
    // home action  .........
	if (action.type === PUSH_NEW_ROUTE_HOME) {
		
        globalHomeNav.navigator.push({id: action.route});
        return {
            routes: [...state.routes, action.route]
        };
    }
	if (action.type === POP_ROUTE_HOME) {
        globalHomeNav.navigator.pop();
        let routes = state.routes;
        routes.pop();
        return {
            routes: routes
